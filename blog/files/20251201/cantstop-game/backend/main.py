...
class GameMechanics:
    ...
    def get_pairing_playability(
        ...
        print(f"  Checking playability for pairing {pairing}")
        print(f"    Active runners: {active_runners} (count: {len(active_runners)})")
        print(f"    All completed: {all_completed}")
        print(f"    Columns at top: {columns_at_top}")
        ...
        if sum1 not in all_completed and sum1 not in columns_at_top:
            if sum1 in active_runners or len(active_runners) < 3:
                sum1_playable = True
        print(f"    sum1={sum1}: playable={sum1_playable} (completed={sum1 in all_completed}, at_top={sum1 in columns_at_top}, active={sum1 in active_runners})")
        ...
        if sum1 != sum2:
            ...
            sum2_playable = sum1_playable
        print(f"    sum2={sum2}: playable={sum2_playable} (completed={sum2 in all_completed}, at_top={sum2 in columns_at_top}, active={sum2 in active_runners})")
        ...
        print(f"    RESULT: needs_choice={needs_choice}, both_can_apply={both_can_apply}")
...
class GameManager:
    ...
    def roll_dice(self, game_id: str) -> dict:
        ...
        print(f"\n=== ROLL DICE ===")
        print(f"Dice: {game.current_dice}")
        print(f"Available pairings: {game.available_pairings}")
        print(f"Active runners: {game.active_runners}")
        ...
        print(f"Columns at top (not yet saved): {columns_at_top}")
        ...
        print(f"Valid pairings: {game.valid_pairings}")
        print(f"Calculating playability for {len(game.valid_pairings)} valid pairings:")

        # Check for bust - keep dice visible, DON'T switch player yet
        if len(game.valid_pairings) == 0:
            game.is_bust = True
            print(f"BUST! No valid pairings")

            # Clear temp progress (lost on bust)
            game.temp_progress = {}
            game.active_runners = set()

            # Keep dice and pairings visible for review
            # DO NOT clear: game.current_dice, game.available_pairings, game.valid_pairings
            ...
            print(f"Player {game.current_player} is busted, but stays as current player for display")
    ...
    def choose_pairing(self, game_id: str, pairing_index: int, chosen_number: Optional[int] = None) -> dict:
        ...
        # Debug logging
        print(f"=== APPLYING PAIRING ===")
        print(f"Chosen pairing: {chosen_pairing}")
        print(f"Chosen number: {chosen_number}")
        print(f"Active runners before: {game.active_runners}")
        print(f"Temp progress before: {game.temp_progress}")
        print(f"All completed: {all_completed}")
        ...
        print(f"Active runners after: {game.active_runners}")
        print(f"Temp progress after: {game.temp_progress}")
        print(f"=======================")
        ...
        game.last_chosen_pairing_index = pairingIdx
