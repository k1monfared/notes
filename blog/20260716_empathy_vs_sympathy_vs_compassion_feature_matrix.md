...
7. **Why "caring" is split into two rows**: a single "~" was doing two incompatible jobs, weaker-in-degree versus full-strength-but-narrow-in-scope. Scope is exactly what separates the sociopath, who can care intensely but only for an in-group, from a healthy person. So "capacity to care at all" and "concern reaches beyond an in-group" are now separate features. The sociopath scores present on the first and absent on the second.
8. **Etiology is deliberately left out as a row**: psychopathy is often theorized as innate and temperamental, sociopathy as environmentally acquired. That is a claim about *causes*, not about the shape of the response, so it does not belong in a feature table. Every other row answers "what is the response like," and mixing in an origin variable would muddy that logic.
9. **Clinical caveat on the two profiles**: "psychopath" and "sociopath" are not formal diagnoses. Both fall under Antisocial Personality Disorder, and the split between them is a contested, largely theoretical convention. Treat these two columns as ideal types, not categories.
10. **Terminology caveat**: usage varies across the literature. Some authors use "empathic concern" roughly where this table uses "compassion," and pop-psych usage (e.g. Brené Brown) treats "sympathy" as inherently distancing. This table uses the more neutral research sense.
